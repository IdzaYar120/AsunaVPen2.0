...
class PetWindow(QWidget):
    def __init__(self):
        ...
        self.emote_label = QLabel(self); self.emote_label.setFixedSize(60, 60); self.emote_label.hide()
        
        self.bar_xp = QProgressBar(self); self.bar_xp.setFixedHeight(4); self.bar_xp.setTextVisible(False)
        self.bar_xp.setStyleSheet("QProgressBar { border: none; background: rgba(0,0,0,80); } QProgressBar::chunk { background: #FFD700; }")
        self.bar_xp.hide()

        self.stats_panel = QWidget(self); self.stats_panel.setFixedWidth(130); self.stats_panel.setFixedHeight(45); self.stats_panel.hide()
        layout = QVBoxLayout(self.stats_panel); layout.setSpacing(2); layout.setContentsMargins(5, 5, 5, 5)
        self.bar_hunger = self.create_bar("#FF4444"); self.bar_energy = self.create_bar("#44FF44"); self.bar_health = self.create_bar("#4444FF")
    ...
    def create_floating_text(self, text, color="#FFD700"):
        ...
        move = QPropertyAnimation(label, b"pos"); move.setDuration(1500)
        move.setStartValue(QPoint(x, y)); move.setEndValue(QPoint(x, y-70))
        ...
        group.finished.connect(group.deleteLater) # ЗАХИСТ ВІД ВИТОКУ
    ...
    def render_pet(self, px):
        ...
        self.emote_label.move((self.width()-60)//2, 10); self.bar_xp.raise_(); self.stats_panel.raise_(); self.show()
    ...
    def contextMenuEvent(self, event):
        if self.engine:
            ...
            menu.addSeparator(); menu.addAction("❌ Вийти").triggered.connect(QApplication.quit)
