...
class SoundWrapper:
    ...
    def init_sound(self):
        ...
        if self.is_mp3:
            ...
            self.player.setSource(url)
            self.audio_output.setVolume(Settings.VOLUME_SFX / 100.0)
        else:
            self.effect = QSoundEffect()
            self.effect.setSource(url)
            self.effect.setVolume(Settings.VOLUME_SFX / 100.0)
...
class SoundManager:
    ...
    def set_global_volume(self, volume_0_100):
        """Update volume for all sounds."""
        Settings.VOLUME_SFX = volume_0_100
        vol_float = volume_0_100 / 100.0
        
        for wrapper in self.sounds.values():
            if wrapper.is_mp3:
                wrapper.audio_output.setVolume(vol_float)
            else:
                wrapper.effect.setVolume(vol_float)
